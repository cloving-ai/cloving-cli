...
export const config = async (): Promise<void> => {
  ...
  while (continueConfig) {
    ...
    const priority = primary ? 100 : parseInt(await promptUser('Enter the priority for this model (0-100, higher priority will be used as long as the prompt fits the context token limit): [' + (primary ? 100 : 0) + ']'), 10)
